	...
	public  void run (String[] args) throws IOException {
		...
		while ((line = responseReader.readLine()) != null) {
			...
			if (!relationProv.equals("NIL")) {
				...
				if (!ignoreoffsets) {
					fillerProv = fields[6].trim(); 
					fillerProv = sortSpans(fillerProv); // TODO: need to sort spans in fillerProv for lenient (unofficial) scoring
				} else if(! anydoc) {
					// TODO: else if ignore offsets: remove offsets from spans, keep just the docs, remove duplicate docs
					fillerProv = removeOffsets(fields[6].trim());
					relationProv = removeOffsets(relationProv);            
				}
				confidence=Double.parseDouble(fields[7].trim());
			...
			if(relationProv.equals("NIL")){
				...
				key = entity+"~"+slot+"~"+ans.toLowerCase();
			...
			if(fields.length>5){
				rel_off = "";
				if(!fields[6].trim().contains(",")){
					rel_off = fields[6].trim().split(":")[0];
				}
				else{
					String[] tmp = fields[6].trim().split(",");
					rel_off = tmp[0].split(":")[0];
				}
				rel_prov.put(key, rel_off);
				off = "";
				if(!fields[6].trim().contains(",")){
					off = fields[6].trim().split(":")[1];
				}
				else{
					String[] tmp = fields[6].trim().split(",");
					off = tmp[0].split(":")[1];
		...
		for (String query : slots) {
			...
			for (String responseString : responseList) {
				...
				if (prov.equals("NIL")) {
					...
					tkey += "~" + fields[2].toLowerCase() ;		  
					String J = judgement.get(key);
					if (J == null) {
						System.out.println ("Warning: No judgement for " + key);
						//mpTarget.put(tkey,0); //added by nazneen
						J = WRONG;
					}
					symbol = J;
